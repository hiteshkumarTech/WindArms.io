...
/**
 * Fallback muzzle offset from the eye, in view space — used whenever
 * `muzzleWorldPose.ready` is false: before `VortexViewmodel`'s first frame
 * (initial load), or during the brief window after a remount before its
 * next frame runs (`VortexViewmodel` resets `ready` to false on both mount
 * and unmount — see `muzzleWorldPose.ts`). Genuinely reachable, not dead
 * code: a shot fired in that window (e.g. a fast restart re-triggering a
 * remount) uses this instead of a stale or uninitialized world position.
 * The real, per-frame, pose-aware muzzle position is `muzzleWorldPose` —
 * see docs/decisions.md "Vortex Rifle FP pose correction". Shell-casing
 * eject keeps this same camera-relative approximation deliberately (not in
 * this pass's scope — only muzzle flash/tracer/smoke were asked for).
 */
...
/**
 * Vortex Rifle fire system — headless, runs entirely in useFrame. MOVED
 * 2026-07-17 (Milestone 6) from three/range/ to three/weapons/ (its true
 * home — this is THE weapon's single implementation, consumed by both
 * `/v2/range` and `/v2/play`; the old path re-exports this file). One
 * addition in that move: the optional `combatGateRef` — a per-frame gate
 * the play route's match state machine drives (fire/reload/inspect/spin-up
 * all blocked while closed). Omitted (the range) ⇒ always open, byte-for-
 * byte the previous behavior. Everything else is unchanged; see the module
 * history in git for the original range-era documentation.
 */
export default function VortexFireSystem({
  inputRef,
  combatGateRef,
}: {
  inputRef: React.MutableRefObject<RangeInputSnapshot>;
  /** Open (true) ⇒ weapon usable. The play route closes it during countdown, death, pause and end screens. */
  combatGateRef?: React.MutableRefObject<boolean>;
}) {
  ...
  useFrame(() => {
    ...
    const hasControl = document.pointerLockElement !== null && (combatGateRef?.current ?? true);
    ...
    // Inspect (F) — same edge-trigger idiom, blocked while reloading.
    if (
      hasControl &&
      !equipping &&
  ...
  function fire(spinUpT: number): void {
    ...
    // Visual muzzle origin (tracer/flash/smoke) — the actual barrel position
    // via the runtime anchor VortexViewmodel publishes every frame, NOT the
    // gameplay aim ray below (which stays camera-based, unaffected by this).
    if (muzzleWorldPose.ready) {
      muzzle.copy(muzzleWorldPose.position);
    } else {
    ...
    // THREE.Sprite.raycast() dereferences raycaster.camera unconditionally —
    // must be set before intersecting a scene containing any visible sprite.
...
/** Walks up from the raycast hit object to find the root carrying `userData.isTarget` (range targets set it on the mesh itself; drones may set it on a child hitbox's ancestor). */
