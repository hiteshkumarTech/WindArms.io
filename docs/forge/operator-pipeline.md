...
**Hand IK — groundwork laid, not yet implemented (2026-07-21).** Both prerequisites the future two-bone IK layer needs now exist, still one step short of actually solving anything: (1) `tools/blender/inspect-kael-hand-basis.py` measures Kael's real `mixamorig:LeftHand`/`RightHand` rest-pose armature-space basis (finger-forward/thumb/palm-normal directions, per-hand — confirmed NOT a simple left/right mirror, see `docs/decisions.md`), and (2) the Vortex Rifle now publishes weapon-owned right/left hand WORLD-SPACE target transforms every frame (`src/lib/v2/weapons/gripWorldPose.ts`), in a coordinate basis deliberately chosen to match (1) directly. Neither the FP arms GLB (`operator-kael-arms.glb`, extracted and validated separately — `docs/forge/kael-v0.1-inspection.md`) nor these targets are connected to each other yet; no IK solver exists, and the rifle is not held. The grip-target VALUES are now final (visually calibrated `/v2/range?grips=1`, 2026-07-21): right hand `position: [-0.25, -0.065, 0.0]` / `rotationEuler: [0.0, 0.0, -1.1519]` (-66° Z); left hand `position: [0.22, -0.05, 0.0]` / `rotationEuler: [0.0, 0.0, -0.5061]` (-29° Z), both `rotationOrder: 'XYZ'`, both temporary runtime proxies not authored GLB sockets. Full detail: `docs/design/weapons/vortex-rifle.md` §22a.
