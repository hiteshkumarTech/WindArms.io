...
0. **Update (2026-07-26, Milestone 8 Steps 8B/8B.1/8C) — item 1 below is stale.** `operator-kael.glb`/`.lod1.glb`, `operator-kael-arms.glb`, and now `operator-kael-lowerbody.glb` have all landed; the procedural silhouette is no longer the only rendered state. This section otherwise reflects its original Phase 5 authoring date and hasn't been swept for currency beyond this note — treat items below as historical unless independently re-verified. Step 8B's own blocker (no Blender executable in this working session's environment) was resolved by running the extraction in a real, live Blender session on the user's own machine — see `docs/decisions.md`'s 2026-07-26 entries for the full trace, including the measured triangle-budget curve that raised the target from 15k-17k to 60k. A follow-up pass, Step 8B.1, then repaired isolated thigh/shin decimation-artifact patches that human visual review caught (a wireframe check on an untextured material) after the initial 60k build — the shipped lower-body derivative is now 58,457 tris / 34,689 verts, down from the originally-built 59,809/35,591, via a targeted post-process cleanup, not a re-extraction. Step 8C then mounted this derivative in both `/v2/range` and `/v2/play` (static integration, world-position/yaw-follow only, no locomotion/shadow yet) — technically complete and validated, but not yet committed pending human visual review of the calibration screenshots; see `docs/decisions.md`'s Step 8C entries for the real-position-vs-capsule-center calibration finding and the honestly-reported eye-to-waist proportion limitation.
