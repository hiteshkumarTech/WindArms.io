...
- **Kael FP-arm two-bone IK: pose calibration DONE (0.00cm both hands) and real-browser motion validated (hip-fire/ADS/30-round-fire/ADS-release, no clipping/detachment). Materials pass (Step 7A) also DONE — realistic PBR gloves/sleeve/armor replace the flat neutral-gray dev material.** Structural blockers resolved and reach deficit fixed in Steps 6E–6G; Step 6H/6I applied full human visual calibration: right hand-basis +30° X, left hand-basis `[30, 30, -10]`, left shoulder assist `[0.06, -0.009, -0.05]`, both wrist-rotation-weights 1.0, left finger curl 0.30 (right finger curl still 0). Real-browser motion test (2026-07-24, not just headless) confirmed no hand detachment/wrist snapping/exploded triangles/scope obstruction through hip-fire, ADS, a full 30-round magazine, and ADS release. Step 7A (2026-07-24) then shipped baked PBR materials (dark charcoal gloves, pale sleeve trim, titanium/gold armor accents, restrained cyan seam) — see [decisions.md](decisions.md) for the region-strategy reasoning and four real pipeline bugs found/fixed along the way. `FIRST_PERSON_ARM_IK_CONFIG` carries every approved IK value as its own shipped default. **What's left**: right-side finger curl (that side's curl stage never reached), and a human visual sign-off on the new materials (screenshots taken and reviewed this pass, but final approval is the user's call — nothing committed). Full detail: [design/weapons/vortex-rifle.md](design/weapons/vortex-rifle.md) §22b, [forge/operator-pipeline.md](forge/operator-pipeline.md) §5, [decisions.md](decisions.md). Runtime body derivatives, the FP-arms extraction, and the grip-anchor target system that this IK layer builds on are all already shipped — see [forge/kael-v0.1-inspection.md](forge/kael-v0.1-inspection.md) and §22a.
- **Deferred from Step 7A (2026-07-24), explicitly out of scope until a later milestone: full-body awareness, an operator showcase, match intro, victory presentation, and third-person spectator systems.** First-person combat is the only gameplay camera the Kael arms/materials work has needed to serve so far — none of the above exist yet in any form and were deliberately not started.
