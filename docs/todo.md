...
- **Kael FP-arm two-bone IK: PARTIALLY visually calibrated by a human browser session (2026-07-23, Step 6H) and saved as the new shipped starting pose — both hands still measure 0.00cm positional error; remaining work is elbow poles, finger curls, and motion validation, in that order.** Structural blockers resolved and reach deficit fixed in Steps 6E–6G (see below); Step 6H applied the first round of human visual calibration: right hand-basis corrected +30° (X, canonical finger-forward axis), left hand-basis needed no adjustment, left shoulder assist refined to `[0.06, -0.009, -0.05]`, both wrist-rotation-weights confirmed at 1.0. Re-verified headlessly against the real asset with these exact shipped values: **RIGHT 0.000cm, LEFT 0.000cm**. Finger curls explicitly NOT yet reached (left at 0 — "not yet tuned," not "confirmed off"). **Next session's approved order**: left-hand basis tuning → elbow poles → finger curls → ADS/recoil check → `/v2/play` re-check. `FIRST_PERSON_ARM_IK_CONFIG` now carries every approved value as its own shipped default (a fresh `?ik=1` tuner session starts from this pose, not from zero) — the panel's own footer (reset/copy-code buttons) had a real CSS viewport-clipping bug fixed in the same pass (`max-h-[85vh]` didn't account for the panel's `top-72` offset, silently clipping the bottom on any normal viewport). Full detail: [design/weapons/vortex-rifle.md](design/weapons/vortex-rifle.md) §22b, [forge/operator-pipeline.md](forge/operator-pipeline.md) §5, [decisions.md](decisions.md). Runtime body derivatives, the FP-arms extraction, and the grip-anchor target system that this IK layer builds on are all already shipped — see [forge/kael-v0.1-inspection.md](forge/kael-v0.1-inspection.md) and §22a.
