...
- **BLOCKER — Kael Aurin source GLB does not exist** (Milestone 7 gate, 2026-07-20): first-person arms, third-person model, operator select and all operator cosmetics are asset-gated on one rigged A-pose GLB at `WindArms Assets/Characters/Operator01_Kael/kael_v0.1_source.glb`. Production order + copy-paste generation/rigging brief: [forge/kael-v0.1-source-brief.md](forge/kael-v0.1-source-brief.md). Everything downstream (slots, manifest, render modes, socket/bone fallbacks, FP rig) already exists and is waiting.
