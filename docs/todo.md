...
- **Kael FP-arm two-bone IK: pose calibration (basis + shoulder assist + first finger curl) is DONE and saved as shipped default; both hands hold 0.00cm positional error through hip-fire/ADS in headless checks — next milestone is HUMAN motion validation (hip-fire, ADS, recoil, reload, inspect, `/v2/play`), not further tuning, unless motion reveals a real problem.** Structural blockers resolved and reach deficit fixed in Steps 6E–6G; Step 6H/6I applied full human visual calibration: right hand-basis +30° X, left hand-basis `[30, 30, -10]`, left shoulder assist `[0.06, -0.009, -0.05]`, both wrist-rotation-weights 1.0, left finger curl 0.30 (right finger curl still 0, that side's curl stage not yet reached). Re-verified headlessly against the real asset with these exact shipped values: **RIGHT 0.000cm, LEFT 0.000cm** at both hip-fire and ADS poses. A headless motion-stability precheck (simulated recoil-punch sweep, hip→ADS blend) found no NaN/solve-failures/discontinuous elbow flips — but this ONLY proves the math stays numerically stable, not that anything looks right on screen; visual clipping/detachment can only be judged by a human watching a rendered frame, which is exactly what the next session is for. `FIRST_PERSON_ARM_IK_CONFIG` carries every approved value as its own shipped default (a fresh `?ik=1` tuner session starts from this pose). The panel's footer (reset/copy-code buttons) had a real CSS viewport-clipping bug, fixed in Step 6H (`max-h-[85vh]` didn't account for the panel's `top-72` offset). Full detail: [design/weapons/vortex-rifle.md](design/weapons/vortex-rifle.md) §22b, [forge/operator-pipeline.md](forge/operator-pipeline.md) §5, [decisions.md](decisions.md). Runtime body derivatives, the FP-arms extraction, and the grip-anchor target system that this IK layer builds on are all already shipped — see [forge/kael-v0.1-inspection.md](forge/kael-v0.1-inspection.md) and §22a.
