...
**Updated 2026-07-22 — Kael's arms are now mounted here too.** A two-bone IK layer (`src/lib/v2/operators/ik/twoBoneIk.ts`, `kaelArmRig.ts`) solves the validated FP-arms derivative (`operator-kael-arms.glb`) toward the weapon's grip targets (`gripWorldPose.ts`, `docs/design/weapons/vortex-rifle.md` §22a–§22b) — first mounted in `/v2/range` on 2026-07-21, extended here on 2026-07-22 using the exact same `KaelFirstPersonArms` component, unmodified. Both hands visibly track the rifle through hip-fire/ADS/movement/recoil/reload/inspect, in both scenes. Neither `VortexViewmodel.tsx` nor `KaelFirstPersonArms.tsx` reads match-phase state, so this behaves identically through pause/countdown/combat/victory/defeat — the arms never freeze or detach independently of the weapon itself. **Honest limitation: not yet visually calibrated.** No browser/live-render access was available for either mounting pass, so `firstPersonArmIkConfig.ts`'s elbow-pole/shoulder-offset/finger-curl values are structurally validated (tests, types, build, route compilation) but not visually tuned against the rendered scene — a `?ik=1` dev tuner exists in `/v2/range` for a future pass to correct this without re-guessing-and-committing blind.
