...
**Honest limitation:** this is a corrected *floating* viewmodel, not a *held* one. Updated 2026-07-21 — a validated FP-arms derivative (`operator-kael-arms.glb`) and weapon-owned right/left hand world-space target transforms (`gripWorldPose.ts`, `docs/design/weapons/vortex-rifle.md` §22a) both now exist, with the grip-target values now finalized and visually calibrated in `/v2/range?grips=1` (right hand `position: [-0.25, -0.065, 0.0]` / `rotationEuler: [0.0, 0.0, -1.1519]`; left hand `position: [0.22, -0.05, 0.0]` / `rotationEuler: [0.0, 0.0, -0.5061]`) — but they are still NOT connected to anything: no IK solver exists, the arms are not mounted in `/v2/play` or `/v2/range`, and there is still no real hand/grip contact or wrist rotation in either scene. True holding needs the future IK layer (tracked in [todo.md](../todo.md)).
