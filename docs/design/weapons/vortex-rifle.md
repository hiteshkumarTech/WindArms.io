...
**Reload/inspect IK weight behavior — superseded by §22c's full action-pose system (Step 7C, 2026-07-25).** The original Step 11 approach (a flat weight reduction to 0.15/0.3 during reload/inspect, `firstPersonArmWeights.ts`'s now-retired `computeArmWeightTargets`) has been replaced by a phase-driven presentation with an actual weapon-relative action target — see §22c.
...
- **IK pose (elbow poles, shoulder offset, hand-basis, finger curl) is fully calibrated and real-browser motion validated as of 2026-07-24–25** — RIGHT 0.00cm / LEFT 0.00cm positional accuracy, re-confirmed live after the Step 7C changes below. Right-side finger curl is the one field never reached its own calibration stage (still 0).
- **Materials shipped 2026-07-24 (Step 7A)** — realistic PBR gloves/sleeve/armor replace the former neutral dev material entirely. See §22 area docs and `docs/changelog.md`'s 2026-07-24 entry.
- **Reload/inspect presentation is procedural (Step 7C, 2026-07-25), not authored animation clips** — see §22c. This is a deliberate, stated design choice (no animation clips exist for this derivative, and none are planned to replace the procedural system at this stage), not a stopgap awaiting clips.
...
- **The two new Step 7C action-target anchors (`RELOAD_LEFT_HAND_LOCAL`/`INSPECT_LEFT_HAND_LOCAL`) are NOT visually calibrated** — reasoned conservative offsets from the calibrated grip anchor, not checked against the running scene. A `?anim=1` "show action target" marker exists for a future calibration pass.
- **`/v2/play` has Kael's arms, materials, AND the Step 7C reload/inspect presentation, all via the exact same components as `/v2/range`, unmodified for that route.** No remaining integration gap between the two scenes.

## 22c. Reload/Inspect Action-Pose Presentation (Milestone 7, Phase G, Step 7C, 2026-07-25)

Procedural (not authored-clip) reload/inspect presentation: the left (support) hand visibly releases the grip, moves to a temporary weapon-relative action target, and returns exactly to §22a/§22b's calibrated grip pose. The right hand and every approved IK/grip/weapon value are completely untouched by this step.

**Architecture — a pure function of normalized time, consumed entirely outside the IK solver.** `computeActionPose()` (`lib/v2/operators/actionPose.ts`) takes `{kind: 'idle'|'reload'|'inspect', progress: 0..1, canonical finger curls}` and deterministically returns a `FirstPersonActionPose`: weapon position/rotation offset, per-hand IK weight, per-hand finger-curl scale, the left hand's action-target blend weight, and a phase name. `VortexViewmodel.tsx` (which already owned reload/inspect progress and the weapon's own dip/wobble) computes this ONCE per frame, applies the weapon-facing half directly to its own transform, and publishes the hand-facing half (`actionPoseState.ts`, same singleton-bridge family as `muzzleWorldPose.ts`/`gripWorldPose.ts`) for `KaelFirstPersonArms.tsx` to read imperatively. The left hand's actual IK TARGET is a lerp/slerp blend between the normal grip anchor and a resolved action-target anchor, computed BEFORE calling the unmodified `solveSide` — the solver itself has zero awareness that an action is happening; it only ever receives a (possibly blended) target, exactly the same contract it always had. Full reasoning for this "blend outside the solver" choice: `docs/decisions.md`.

**Phases.** Reload (duration unchanged, `reloadTimeS` from `shared/windWeapons.ts`): `reloadSettle` (progress 0–0.3, hand starts moving toward the action target) → `reloadManipulate` (0.3–0.7, held at the action target) → `reloadReturn` (0.7–1.0, back to the grip). Inspect (duration unchanged, 1.5s): `inspectAnticipate` (0–0.15) → `inspectHold` (0.15–0.75) → `inspectReturn` (0.75–1.0). The weapon's reload dip is the exact pre-existing curve, ported verbatim. The weapon's inspect motion was redesigned (not ported) into a held ~26° yaw / ~7° roll / 0.05m lift that actually reveals the rifle's side profile during the hold, per the brief's explicit ask — see `docs/decisions.md` for why reload's weapon motion was left alone while inspect's was redeveloped.

**Action-target anchors** (`lib/v2/weapons/actionTargets.ts`, `RELOAD_LEFT_HAND_LOCAL`/`INSPECT_LEFT_HAND_LOCAL`) — same `RuntimeGripAnchor` shape/coordinate convention as §22a's grip anchors, small (6–8cm) offsets from the already-calibrated `gripSupportLocal` rather than a fabricated magazine-well location (the real mesh has no separate magazine/bolt geometry to align a hand against). **Not visually calibrated** — flagged the same way every temporary runtime anchor in this codebase is flagged, with a `?anim=1` "show action target" marker for a future human pass.

**IK weight and finger curl during actions.** Right hand: `rightHandIkWeight` stays exactly `1.0` and its finger curl stays exactly canonical in every phase of both actions — there is no code path that could vary it, not just a runtime check. Left hand: IK weight softens only slightly (max ~12% during reload, ~10% during inspect) — a controlled, purposeful movement toward a real target, not a loose/floating release toward rest pose. Finger curl tightens above canonical during reload's manipulation hold (reads as gripping/working the action) and relaxes below canonical during inspect's hold (reads as an open, sliding hand) — both fully return to the exact canonical value by phase completion (verified byte-identical in both unit tests and real-browser sampling after 5 consecutive reloads and 5 consecutive inspects — zero drift).

**Pause/death.** `VortexViewmodel.tsx` now also checks `useV2MatchStore`'s phase (imperative `getState()` read, safe on `/v2/range` too, where match phase never leaves its initial state) and folds `paused`/`playerDead` into the SAME freeze mechanism the grip-tuner's own "frozen" toggle already used — reusing proven code, not a new mechanism. Verified in a real browser: two screenshots taken 1 real second apart while genuinely paused (via `document.exitPointerLock()`, which correctly triggers the same `pointerlockchange`-driven pause path a real Escape press does) are byte-for-byte identical.

**Dev tooling — `/v2/range?anim=1`.** Same `NODE_ENV`-gated + explicit-query-param contract as `?ik=1`/`?grips=1`. `ArmActionDebugPanel.tsx` (play reload/inspect, scrub progress, freeze/resume, loop, live phase/weight/curl readout, copy action-target config) and `ActionTargetDebugMarkers.tsx` (3D markers for the action target and the normal grip target). Scrub mode overrides only the `kind`/`progress` fed into `computeActionPose` for that frame — never touches ammo or the real `reloadingUntil`/`inspectingUntil` gameplay state, so it cannot affect real firing/reload logic even while active. Absent from `/v2/play` and from production.

**Validation.** 31 new deterministic unit tests (phase boundaries, progress clamping, exact canonical recovery, no drift after 200 repeated calls, frame-rate independence, finite output). Real browser: both actions confirmed progressing through their real named phases end-to-end; 5 consecutive reloads and 5 consecutive inspects all completed with zero drift in the recovered weights/curls; interruption matrix (ADS-attempt during reload, sprint during reload, inspect-from-hip-fire, fire-after-inspect) all correct; `/v2/play` reload/inspect/pause all confirmed working via the identical component. Live `?ik=1` re-check after all changes: **RIGHT 0.00cm, LEFT 0.00cm** — unchanged. `typecheck`/`test` (200/200)/`lint`/`build` clean; both GLB inspectors pass unchanged (no asset touched by this step).

**Found, not fixed: a genuine pre-existing bug in `VortexFireSystem.tsx`** (unrelated to this step's own code, discovered by this step's interruption-matrix testing) — a queued trigger press during reload isn't cleared on the "blocked because reloading" exit path, so a single click during reload fires a phantom shot the instant the reload completes even after the player releases the button. Left unfixed per this pass's "do not change gameplay files" scope; full root cause and the one-line fix: `docs/decisions.md`.
