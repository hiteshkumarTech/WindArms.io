...
**Step 7G (2026-07-26) — signature shot identity, first pass toward the content gap above.** `src/lib/v2/weapons/vortexSoundRecipe.ts` (pure, `node:test`-covered — 25 tests) now defines a four-layer shot recipe directly mapped to this section's five categories: a compressed-noise **pressure discharge** (primary body — "pressure release"), a short pitched **mechanical transient** (timing clarity, reads as valve/gate movement), a brief downward-sweeping **electromagnetic snap** (kept subtle, not a laser chirp), and a filtered-air **wind tail** ("wind resonance," restrained). A persistent, single-node-chain **turbine layer** (`computeVortexTurbineTarget`) builds and releases with sustained fire, driven by the same `spinUpT` signal `VortexFireSystem.tsx` already computes — the closest current analogue to the brief's "deep mechanical turbine spin." Not yet attempted: the "electromagnetic crack" as a genuinely distinct textural layer from the pressure body (currently folded into the EM-snap's brief sweep), and any of the *environmental* ambience (distant thunder, storm engines, cable tension) — those remain fully open.

**Human listening status (Step 7G):** technically complete — architecture, Web Audio node lifecycle, voice bounding, and offline peak/RMS measurement are all verified (see `../changelog.md`/`../decisions.md` for numbers) — but **not yet creatively approved by a human ear**. Exposed at `/v2/range?audio=1` (single/burst/full-magazine fire, reload, dry-fire, turbine release, offline measurement readouts). Acceptance checklist for that listening pass: does one shot feel powerful and wind/pressure-powered (not a generic laser)? does automatic fire build a distinctive turbine character? does release feel clean? does reload sound mechanically believable without implying a magazine the model doesn't have? is dry-fire clearly, unmistakably weaker? is the volume comfortable, and does sustained fire stay pleasant rather than fatiguing? Until answered, do not describe this as final sound design.
